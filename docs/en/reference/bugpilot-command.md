...
`bugpilot` is the main command for BugPilot, used to start interactive sessions or execute single queries.
...
| `--debug` | | Log debug information (output to `~/.bugpilot/logs/bugpilot.log`) |
...
| `--config-file PATH` | Load configuration file (default `~/.bugpilot/config.toml`) |
...
| `--acp` | Run in ACP server mode (deprecated, use `bugpilot acp` instead) |
...
Default loads `~/.bugpilot/mcp.json` (if exists). See [Model Context Protocol](../customization/mcp.md) for details.
...
|------------|-------------|
| [`bugpilot login`](#bugpilot-login) | Log in to your BugPilot account |
| [`bugpilot logout`](#bugpilot-logout) | Log out from your BugPilot account |
| [`bugpilot info`](./bugpilot-info.md) | Display version and protocol information |
| [`bugpilot acp`](./bugpilot-acp.md) | Start multi-session ACP server |
| [`bugpilot mcp`](./bugpilot-mcp.md) | Manage MCP server configuration |
| [`bugpilot plugin`](../customization/plugins.md) | Manage plugins (Beta) |
| [`bugpilot term`](./bugpilot-term.md) | Launch the Toad terminal UI |
| [`bugpilot export`](#bugpilot-export) | Export a session as a ZIP file |
| [`bugpilot vis`](./bugpilot-vis.md) | Launch the Agent Tracing Visualizer (Technical Preview) |
| [`bugpilot web`](./bugpilot-web.md) | Start the Web UI server |

### `bugpilot login`

Log in to your BugPilot account. This automatically opens a browser; complete account authorization and available models will be automatically configured.
...
Log out from your BugPilot account. This clears stored OAuth credentials and removes related configuration from the config file.
...
```sh
bugpilot export [<session_id>] [-o <output_path>] [--yes]
```
...
See [Agent Tracing Visualizer](./bugpilot-vis.md) for details.
...
# Specify port
bugpilot web --port 8080

# Don't automatically open browser
bugpilot web --no-open

# Bind to all network interfaces (allow LAN access)
bugpilot web --host 0.0.0.0
```

See [Web UI](./bugpilot-web.md) for details.
