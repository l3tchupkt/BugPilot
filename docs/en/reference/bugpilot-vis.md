...
Run `bugpilot vis` in the terminal to start the Visualizer:
...
```sh
# Specify port
bugpilot vis --port 8080

# Don't automatically open browser
bugpilot vis --no-open
...
- **CLI export**: Use `bugpilot export [<session_id>]` to export a session as a ZIP file; when `<session_id>` is omitted, the CLI previews the previous session for the current working directory and asks for confirmation

### Session import

Supports importing ZIP-format session data into the Visualizer for viewing. Imported sessions are stored in a dedicated `~/.bugpilot/imported_sessions/` directory, separate from regular sessions.
