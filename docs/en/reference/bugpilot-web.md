...
Web UI provides a browser-based interactive interface, allowing you to use all features of BugPilot in a web page. Compared to the terminal interface, Web UI offers a richer visual experience, more flexible session management, and more convenient file operations.

## Starting Web UI

Run the `bugpilot web` command in your terminal to start the Web UI server:
...
bugpilot web --network

# Bind to a specific IP address
bugpilot web --host 192.168.1.100
```
...
```sh
bugpilot web --network --auth-token my-secret-token
```
...
```sh
bugpilot web --network --allowed-origins "https://example.com,https://app.example.com"
```
...
```sh
bugpilot web --network --public --auth-token my-secret-token
```
...
```sh
bugpilot web --network --restrict-sensitive-apis
```
...
```sh
bugpilot web --dangerously-omit-auth
```
...
If you are using BugPilot in shell mode in the terminal, you can enter the `/web` command to quickly switch to Web UI:

```
/web
```

After execution, BugPilot will automatically start the Web UI server and open the current session in the browser. You can continue the conversation in Web UI, and the session history will remain synchronized.
...
```sh
bugpilot web --network --auth-token $(openssl rand -hex 32)
```
...
Web UI communicates with BugPilot's Wire mode via WebSocket, enabling real-time bidirectional data transmission.
