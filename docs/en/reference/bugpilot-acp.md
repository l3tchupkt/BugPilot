# `bugpilot acp` Subcommand

The `bugpilot acp` command starts a multi-session ACP (Agent Client Protocol) server.
...
For using BugPilot in IDEs, see [Using in IDEs](../guides/ides.md).
...
Upon receiving this error, the client should guide the user to run the `bugpilot login` command in the terminal to complete login. Once logged in, subsequent ACP requests will proceed normally.
