...
以下环境变量在使用 `bugpilot` 类型的供应商时生效，用于覆盖供应商和模型配置。

| 环境变量 | 说明 |
| --- | --- |
| `BUGPILOT_BASE_URL` | API 基础 URL |
| `BUGPILOT_API_KEY` | API 密钥 |
| `BUGPILOT_MODEL_NAME` | 模型标识符 |
| `BUGPILOT_MODEL_MAX_CONTEXT_SIZE` | 最大上下文长度（token 数） |
| `BUGPILOT_MODEL_CAPABILITIES` | 模型能力，逗号分隔（如 `thinking,image_in`） |
| `BUGPILOT_MODEL_TEMPERATURE` | 生成参数 `temperature` |
| `BUGPILOT_MODEL_TOP_P` | 生成参数 `top_p` |
| `BUGPILOT_MODEL_MAX_COMPLETION_TOKENS` | `max_completion_tokens` 的显式硬上限 |
| `BUGPILOT_MODEL_MAX_TOKENS` | `BUGPILOT_MODEL_MAX_COMPLETION_TOKENS` 的兼容别名 |
| `BUGPILOT_MODEL_THINKING_KEEP` | BugPilot `thinking.keep` 开关（Preserved Thinking），仅在 Thinking 模式下生效 |
...
`BUGPILOT_MODEL_MAX_TOKENS` 仍可使用；如果两个环境变量都设置，优先使用
`BUGPILOT_MODEL_MAX_COMPLETION_TOKENS`。将任一变量设为 `0` 或负整数可禁用 completion token
限制。

### `BUGPILOT_MODEL_THINKING_KEEP`

将 env 值原样作为 `thinking.keep` 字段发送给 BugPilot API，用于开启 Preserved Thinking（参考 [BugPilot 官方文档](https://platform.bugpilot.com/docs/guide/use-bugpilot-k2-thinking-model#preserved-thinking)）。设为 `all` 可让模型在多轮之间保留历史 reasoning_content。值不做任何校验、不做大小写归一化，透传给 API 自己判断。
...
此参数仅在支持 Preserved Thinking 的 BugPilot 模型（例如 `bugpilot-k2.6` / `bugpilot-k2-thinking`）上生效。传给其它模型时，BugPilot API 会忽略或拒绝该字段，CLI 本身不做校验。
...
| 环境变量 | 说明 |
| --- | --- |
| `BUGPILOT_SHARE_DIR` | 自定义共享目录路径（默认 `~/.bugpilot`） |
| `BUGPILOT_NO_AUTO_UPDATE` | 禁用所有更新相关功能 |
| `BUGPILOT_PASTE_CHAR_THRESHOLD` | 粘贴文本折叠的字符数阈值（默认 `1000`） |
| `BUGPILOT_PASTE_LINE_THRESHOLD` | 粘贴文本折叠的行数阈值（默认 `15`） |

### `BUGPILOT_SHARE_DIR`

自定义 BugPilot 的共享目录路径。默认路径为 `~/.bugpilot`，配置、会话、日志等运行时数据存储在此目录下。
...
::: warning 注意
`BUGPILOT_SHARE_DIR` 不影响 [Agent Skills](../customization/skills.md) 的搜索路径。Skills 是跨工具共享的能力扩展（与 Claude、Codex 等兼容），与应用运行时数据是不同类型的数据。如需覆盖 Skills 路径，请使用 `--skills-dir` 参数。
:::
