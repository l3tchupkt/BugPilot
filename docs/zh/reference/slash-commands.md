...
斜杠命令是 BugPilot 的内置命令，用于控制会话、配置和调试。在输入框中输入 `/` 开头的命令即可触发。
...
提交反馈以改进 BugPilot。执行后会提示输入反馈内容并提交。如果网络请求失败或超时，会自动回退到打开 GitHub Issues 页面。
...
登出当前平台。会清理存储的凭据并移除配置文件中的相关配置。登出后 BugPilot 会自动重新加载配置。
...
切换终端配色主题。BugPilot 提供深色（`dark`）和浅色（`light`）两套配色方案，默认使用深色主题。
...
创建一个新会话并立即切换过去，无需退出 BugPilot。如果当前会话没有任何内容，会自动清理空会话目录。
...
回退到之前的某个轮次并重试。执行后会弹出交互式选择器，展示所有历史轮次的用户消息（截断到 80 字符）。选中某个轮次后，BugPilot 会 fork 出一个新会话，包含该轮次**之前**的所有对话历史，并将被选中轮次的用户消息预填到输入框，方便编辑后重新发送。原会话始终保留不丢失。
...
当上下文过长时，BugPilot 会自动触发压缩。此命令可手动触发压缩过程。
