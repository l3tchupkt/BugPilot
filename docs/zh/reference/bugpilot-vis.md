...
在终端中运行 `bugpilot vis` 命令启动 Visualizer：
...
- **CLI 导出**：使用 `bugpilot export [<session_id>]` 命令导出会话为 ZIP 文件；省略 `<session_id>` 时会预览并确认当前工作目录的上一个会话

### 会话导入

支持将 ZIP 格式的会话数据导入到 Visualizer 中查看。导入的会话存储在独立的 `~/.bugpilot/imported_sessions/` 目录中，不会与正常会话混淆。
