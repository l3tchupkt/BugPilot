...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io/) 是一个开放协议，让 AI 模型可以安全地与外部工具和数据源交互。BugPilot 支持连接 MCP 服务器，扩展 AI 的能力。

## MCP 是什么

MCP 服务器提供「工具」给 AI 使用。例如，一个数据库 MCP 服务器可以提供查询工具，让 AI 能够执行 SQL 查询；一个浏览器 MCP 服务器可以让 AI 控制浏览器进行自动化操作。

BugPilot 内置了一些工具（文件读写、Shell 命令、网页抓取等），通过 MCP 你可以添加更多工具，比如：
...
BugPilot 对敏感操作（如文件修改、命令执行）会请求用户确认。MCP 工具也遵循同样的审批机制，所有 MCP 工具调用都会弹出确认提示。

**提示词注入风险**

MCP 工具返回的内容可能包含恶意指令，试图诱导 AI 执行危险操作。BugPilot 会对工具返回内容进行标记，帮助 AI 区分工具输出和用户指令，但你仍应：
