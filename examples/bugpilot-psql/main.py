"""
bugpilot-psql: AI-assisted PostgreSQL interactive terminal.
...
class PsqlShell:
    """Main TUI orchestrator for bugpilot-psql."""
    ...
    def _create_prompt_session(self) -> PromptSession[str]:
        ...
        def get_prompt() -> FormattedText:
            ...
            return FormattedText([("bold fg:blue", f"bugpilot-psql{symbol} ")])
    ...
    def _print_welcome(self) -> None:
        ...
        console.print(
            Panel(
                Text.from_markup(
                    "[bold]Welcome to bugpilot-psql![/bold]\n"
    ...
    async def _run_ai_mode(self) -> None:
        ...
        try:
            ...
            console.print("[red]LLM not set, run `bugpilot /setup` to configure[/red]")
...
def main(
    ...
    """
    Start bugpilot-psql: AI-assisted PostgreSQL interactive terminal.

    Usage is compatible with psql:
      bugpilot-psql mydb
      bugpilot-psql mydb postgres
      bugpilot-psql -h localhost -U postgres -d mydb
      bugpilot-psql --conninfo postgresql://user:pass@host/db
    """
...
async def _run_async(
    ...
    if not model:
        model = LLMModel(provider="bugpilot", model="", max_context_size=250_000)
    if not provider:
        provider = LLMProvider(type="bugpilot", base_url="", api_key=SecretStr(""))

    # Override with environment variables
    env_overrides = augment_provider_with_env_vars(provider, model)

    if not provider.base_url or not model.model:
        console.print("[red]LLM not configured. Run `bugpilot /setup` to configure.[/red]")
        return
