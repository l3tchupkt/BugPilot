...
from bugpilot.hooks.config import HookDef
from bugpilot.llm import ModelCapability, ProviderType
...
class Config(BaseModel):
    ...
    provider: ActiveProviderConfig | None = Field(
        default=None, description="Active provider selection"
    )
    ...
    merge_all_available_skills: bool = Field(
        default=True,
        description=(
            "Merge skills from all existing brand directories (bugpilot/claude/codex) "
    ...
    telemetry: bool = Field(
        default=True,
        description="Enable anonymous telemetry to help improve bugpilot. Set to false to disable.",
    )
    ...
    def validate_model(self) -> Self:
        ...
        if self.provider:
            if self.provider.name not in self.providers:
                raise ValueError(f"Active provider {self.provider.name} not found in providers")
        return self
