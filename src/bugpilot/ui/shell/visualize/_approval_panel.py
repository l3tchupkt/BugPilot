...
from bugpilot.utils.rich.diff_render import (
    collect_diff_hunks,
    render_diff_panel,
    render_diff_preview,
...
class ApprovalRequestPanel:
            ...
            if isinstance(block, DiffDisplayBlock):
                ...
                if non_diff_budget > 0:
                    ...
                    if line_count > non_diff_budget:
                        truncated = "\n".join(text.split("\n")[:non_diff_budget])
                        self._non_diff_truncated = True
                    self._preview_renderables.append(BugPilotSyntax(truncated, block.language))
                    non_diff_budget -= min(line_count, non_diff_budget)
                else:
                    self._non_diff_truncated = True
                idx += 1
    ...
    def _render_block(
        ...
        if block.lexer:
            return BugPilotSyntax(text, block.lexer)
    ...
    with console.screen(), console.pager(styles=True):
        ...
        while idx < len(display):
            ...
            if isinstance(block, DiffDisplayBlock):
                ...
                console.print(BugPilotSyntax(block.command.rstrip("\n"), block.language))
                rendered_any = True
                idx += 1
            elif isinstance(block, BriefDisplayBlock) and block.text:
