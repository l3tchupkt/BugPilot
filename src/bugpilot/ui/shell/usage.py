...
from bugpilot.utils.datetime import format_duration
...
async def usage(app: Shell, args: str):
    ...
    assert isinstance(app.soul, Agent)
    ...
    if usage_url is None:
        console.print("[yellow]Usage is available on BugPilot platform only.[/yellow]")
        return
    ...
    with console.status("[cyan]Fetching usage...[/cyan]"):
        ...
        try:
            ...
            if e.status == 401:
                ...
                message = "Usage endpoint not available. Try BugPilot for Coding."
