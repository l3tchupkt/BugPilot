...
from bugpilot.background import list_task_views
from bugpilot.llm import model_display_name
from bugpilot.notifications import NotificationManager, NotificationWatcher
from bugpilot.soul import LLMNotSet, LLMNotSupported, MaxStepsReached, RunCancelled, Soul, run_soul
from bugpilot.soul.agent_loop import FLOW_COMMAND_PREFIX, Agent
from bugpilot.ui.shell import update as _update_mod
from bugpilot.ui.shell.console import console
from bugpilot.ui.shell.echo import render_user_echo_text
from bugpilot.ui.shell.mcp_status import render_mcp_prompt
from bugpilot.ui.shell.migration_nudge import print_migration_goodbye
...
from bugpilot.ui.shell.replay import replay_recent_history
from bugpilot.ui.shell.slash import SKILL_COMMAND_PREFIX, shell_mode_registry
from bugpilot.ui.shell.slash import registry as shell_slash_registry
from bugpilot.ui.shell.update import LATEST_VERSION_FILE, UpdateResult, do_update, semver_tuple
from bugpilot.ui.shell.visualize import (
    ApprovalPromptDelegate,
    visualize,
)
from bugpilot.utils.aioqueue import QueueShutDown
from bugpilot.utils.envvar import get_env_bool
from bugpilot.utils.logging import open_original_stderr
from bugpilot.utils.signals import install_sigint_handler
from bugpilot.utils.slashcmd import SlashCommand, SlashCommandCall, parse_slash_command_call
from bugpilot.utils.subprocess_env import get_clean_env
from bugpilot.utils.term import ensure_new_line, ensure_tty_sane
from bugpilot.wire.types import (
    ApprovalRequest,
    ApprovalResponse,
    ContentPart,
    StatusUpdate,
...
class Shell:
        ...
        panel = Panel(
            Group(
                ...
                Text(
                    ...
                    "Restart bugpilot in a valid directory to continue.",
                    style="dim",
                ),
            ),
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        if isinstance(self.soul, Agent):
            from bugpilot.ui.theme import set_active_theme
        ...
        if get_env_bool("BUGPILOT_CLI_NO_AUTO_UPDATE"):
            logger.info("Auto-update disabled by BUGPILOT_CLI_NO_AUTO_UPDATE environment variable")
        else:
            self._start_background_task(self._auto_update())

        _print_welcome_info(self.soul.name or "BugPilot", self._welcome_info)
...
#         _telemetry_sink = get_sink()
        ...
        def _mcp_status_block(columns: int):
            if not isinstance(self.soul, Agent):
                return None
        ...
        with CustomPromptSession(
            ...
            model_name=model_display_name(
                ...
                if isinstance(self.soul, Agent) and self.soul.runtime.llm
                else None,
            ),
            ...
            editor_command_provider=lambda: (
                self.soul.runtime.config.default_editor if isinstance(self.soul, Agent) else ""
            ),
            ...
            try:
                while True:
                    ...
                    from bugpilot.ui.shell.visualize import InputAction, classify_input
                    ...
                    if action.kind == InputAction.BTW and isinstance(self.soul, Agent):
...
#         track("input_command", command=available_command.name)
        ...
        try:
            ...
            await run_soul(
                ...
                lambda wire: visualize(
                    ...
                    steer=self.soul.steer if isinstance(self.soul, Agent) else None,
            ...
            if not isinstance(e, APIStatusError) or e.status_code not in (401, 402, 403):
                console.print(
                    "[dim]If this persists, run [bold]bugpilot export[/bold] and send the "
            ...
            console.print(
                ...
                "[dim]Run [bold]bugpilot export[/bold] and send the exported data to support "
    ...
    async def _run_btw_modal(
        ...
        """
        from bugpilot.soul.btw import execute_side_question
        from bugpilot.ui.shell.visualize import (
            _BtwModalDelegate,  # pyright: ignore[reportPrivateUsage]
        )

        assert isinstance(self.soul, Agent)
    ...
    def _set_active_view(self, view: Any) -> None:
        ...
        while self._pending_approval_requests:
            ...
            if not isinstance(self.soul, Agent) or self.soul.runtime.approval_runtime is None:
                break
            record = self.soul.runtime.approval_runtime.get_request(request.id)
            if record is None or record.status != "pending":
                continue
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    head = Text.from_markup("Welcome to BugPilot!")
    ...
    if LATEST_VERSION_FILE.exists():
        from bugpilot.constant import VERSION as current_version
        from bugpilot.ui.shell.update import SKIPPED_VERSION_FILE
        from bugpilot.utils.envvar import get_env_bool

        if not get_env_bool("BUGPILOT_CLI_NO_AUTO_UPDATE"):
            try:
