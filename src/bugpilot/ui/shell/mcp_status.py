...
from bugpilot.ui.theme import get_mcp_prompt_colors
from bugpilot.utils.rich.columns import BulletColumns
from bugpilot.wire.types import MCPServerSnapshot, MCPStatusSnapshot
...
def _prompt_server_detail(server: MCPServerSnapshot) -> str:
    if server.status == "unauthorized":
        return f" (unauthorized - run: bugpilot mcp auth {server.name})"
