...
_INSTALL_SH = "curl -fsSL https://code.bugpilot.com/bugpilot-code/install.sh | bash"
_INSTALL_PS = "irm https://code.bugpilot.com/bugpilot-code/install.ps1 | iex"
...
def verify_command(platform: str) -> str:
    """Command to check which `bugpilot` resolves on PATH (Windows: where, else: which)."""
...
def welcome_card_text() -> Text:
    """Welcome-screen card nudging users to upgrade (shown when BugPilot is NOT installed)."""
    return Text.assemble(
        "The new BugPilot is here — rebuilt to be faster and more powerful.\n",
        "Run ",
        ("/upgrade", "bold"),
...
def already_installed_text(platform: str) -> Text:
    """Welcome-screen note shown when BugPilot IS already installed on this machine."""
    return Text.assemble(
        "The new BugPilot is already installed. Start it in a fresh terminal with ",
        ("bugpilot", "bold"),
        " (verify: ",
...
def exit_nudge_text(platform: str) -> Text:
    ...
    return Text.assemble(
        ("Tip: ", "yellow"),
        "The new BugPilot is rebuilt to be faster and more powerful.\n",
        "Install: ",
...
def print_migration_goodbye(
    ...
    *,
    ...
    Skipped entirely (only "Bye!") if BugPilot is already installed.
