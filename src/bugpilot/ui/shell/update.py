...
CHANGELOG_URL_ZH = "https://l3tchupkt.github.io/bugpilot/zh/release-notes/changelog.html"
CHANGELOG_URL_EN = "https://l3tchupkt.github.io/bugpilot/en/release-notes/changelog.html"
...
def _run_update_gate(current_version: str, latest_version: str) -> None:
    ...
    console.print(
        Panel(
            body,
            title="[bold]bugpilot update available[/bold]",
            border_style="yellow",
            expand=False,
            padding=(1, 2),
        )
    ...
    if key in ("\r", "\n"):
        ...
        if result.returncode == 0:
            console.print("[green]Upgrade complete! Run bugpilot to start the new version.[/green]")
        else:
            console.print("[red]Upgrade failed. Please try running manually:[/red]")
            console.print(f"  {UPGRADE_COMMAND}")
...
async def _do_update(*, print: bool, check_only: bool) -> UpdateResult:
    ...
    async with new_client_session(timeout=download_timeout) as session:
        ...
        filename = f"bugpilot-{latest_version}-{target}.tar.gz"
        download_url = f"{BASE_URL}/{latest_version}/{filename}"

        with tempfile.TemporaryDirectory(prefix="bugpilot-") as tmpdir:
            ...
            try:
                ...
                for root, _, files in os.walk(tmpdir):
                    if "bugpilot" in files:
                        binary_path = os.path.join(root, "bugpilot")
                        break
                if not binary_path:
                    logger.error("Binary 'bugpilot' not found in archive.")
                    _print("[red]Binary 'bugpilot' not found in archive.[/red]")
                    return UpdateResult.FAILED
            except Exception:
            ...
            dest_path = INSTALL_DIR / "bugpilot"
    ...
    _print("[yellow]Restart BugPilot to use the new version.[/yellow]")
