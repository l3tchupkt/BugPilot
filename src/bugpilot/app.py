...
from bugpilot.background.models import is_terminal_status
from bugpilot.cli import InputFormat, OutputFormat
from bugpilot.config import Config, LLMModel, LLMProvider, load_config
from bugpilot.llm import augment_provider_with_env_vars, model_display_name
...
from bugpilot.utils.path import shorten_home
from bugpilot.wire import Wire, WireUISide
from bugpilot.wire.types import ApprovalRequest, ApprovalResponse, ContentPart, WireMessage
...
def _patch_session_id(record: dict[str, Any]) -> None:
    ...
    try:
        from bugpilot.soul.toolset import get_session_id
...
def enable_logging(debug: bool = False, *, redirect_stderr: bool = True) -> None:
    ...
    logger.enable("bugpilot")
    if debug:
        logger.enable("kosong")
    logger.add(
        get_share_dir() / "logs" / "bugpilot.log",
...
class BugPilotCLI:
    ...
    async def create(
        ...
        *,
        ...
        Raises:
            ...
            ConfigError(BugPilotError, ValueError): When the configuration is invalid.
            AgentSpecError(BugPilotError, ValueError): When the agent specification is invalid.
            SystemPromptTemplateError(BugPilotError, ValueError): When the system prompt
                template is invalid.
            InvalidToolError(BugPilotError, ValueError): When any tool cannot be loaded.
            MCPConfigError(BugPilotError, ValueError): When any MCP configuration is invalid.
            MCPRuntimeError(BugPilotError, RuntimeError): When any MCP server cannot be
                connected.
        """
        ...
        if not model or not provider:
            ...
            provider = LLMProvider(type="openai-compatible", base_url="", api_key=SecretStr(""))
        ...
        return BugPilotCLI(soul, runtime, env_overrides, bg_refresh_task)
    ...
    async def shutdown_background_tasks(self) -> None:
        ...
        try:
            ...
            if fresh_targets:
                ...
                # ``bugpilot.log`` because ``redirect_stderr_to_logger`` has
    ...
    async def run(
        ...
        """
        Run the BugPilot instance without any UI and yield Wire messages directly.
        ...
        Yields:
            WireMessage: The Wire messages from the `Agent`.
    ...
    async def run_shell(
        ...
        """Run the BugPilot instance with shell UI."""
        from bugpilot.ui.shell import Shell, WelcomeInfoItem

        if command is None:
            from bugpilot.ui.shell.update import check_update_gate
        ...
        if base_url := self._env_overrides.get("BUGPILOT_BASE_URL"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API URL",
                    value=f"{base_url} (from BUGPILOT_BASE_URL)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        if self._env_overrides.get("BUGPILOT_API_KEY"):
            welcome_info.append(
                WelcomeInfoItem(
                    name="API Key",
                    value="****** (from BUGPILOT_API_KEY)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        ...
        elif "BUGPILOT_MODEL_NAME" in self._env_overrides:
            welcome_info.append(
                WelcomeInfoItem(
                    name="Model",
                    value=f"{self._soul.model_name} (from BUGPILOT_MODEL_NAME)",
                    level=WelcomeInfoItem.Level.WARN,
                )
            )
        else:
    ...
    async def run_print(
        ...
        *,
        ...
        """Run the BugPilot instance with print UI."""
    ...
    async def run_acp(self) -> None:
        """Run the BugPilot instance as ACP server."""
        from bugpilot.ui.acp import ACP
    ...
    async def run_wire_stdio(self) -> None:
        """Run the BugPilot instance as Wire server over stdio."""
