...
def _resolve_source(target: str) -> tuple[Path, Path | None]:
    ...
    if parsed.scheme in ("http", "https") and parsed.path.lower().endswith(".zip"):
        ...
        tmp = Path(tempfile.mkdtemp(prefix="bugpilot-plugin-"))
    ...
    if target.startswith(("https://", "git@", "http://")) and (
        ...
        if available:
            ...
            typer.echo(
                ...
                f"Available plugins:\n{names}\n"
                f"Use: bugpilot plugin install <url>/<plugin-name>",
                err=True,
            )
        else:
            typer.echo(
...
def install_cmd(
    ...
    from bugpilot.plugin.manager import get_plugins_dir, install_plugin
    ...
    try:
        ...
        if not host_values.get("api_key"):
            typer.echo(
                ...
                "Run 'bugpilot login' or configure a provider first.",
                err=True,
            )
...
def list_cmd() -> None:
    ...
    from bugpilot.plugin.manager import get_plugins_dir, list_plugins
...
def remove_cmd(
    ...
    from bugpilot.plugin.manager import get_plugins_dir, remove_plugin
...
def info_cmd(
    ...
    """Show plugin details."""
    from bugpilot.plugin import parse_plugin_json
