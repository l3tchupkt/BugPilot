...
from bugpilot.wire.file import WireFileMetadata, parse_wire_file_line
from bugpilot.wire.types import TurnBegin
...
def _collect_recent_log_files(session_dir: Path) -> list[Path]:
    ...
    for f in log_dir.iterdir():
        if not f.is_file() or not f.name.startswith("bugpilot.") or not f.name.endswith(".log"):
            continue
        try:
            st = f.stat()
            mtime = st.st_mtime
...
def _build_manifest(session_id: str, session_dir: Path) -> dict[str, str | None]:
    ...
    try:
        ...
        manifest["bugpilot_version"] = get_version()
