...
cli = typer.Typer(
    ...
    epilog="""\b\
Documentation:        https://l3tchupkt.github.io/bugpilot/\n
LLM friendly version: https://l3tchupkt.github.io/bugpilot/llms.txt""",
    add_completion=False,
    context_settings={"help_option_names": ["-h", "--help"]},
    help="BugPilot, your next CLI agent.",
)
...
def _version_callback(value: bool) -> None:
    if value:
        ...
        typer.echo(f"bugpilot, version {get_version()}")
        raise typer.Exit()
...
def bugpilot(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Config TOML/JSON file to load. Default: ~/.bugpilot/config.toml.",
        ),
    ] = None,
    ...
    acp_mode: Annotated[
        ...
        typer.Option(
            "--acp",
            help="(Deprecated, use `bugpilot acp` instead) Run as ACP server.",
        ),
    ] = False,
    wire_mode: Annotated[
        bool,
    ...
    """BugPilot, your next CLI agent."""
    import asyncio
    import contextlib
    import json

    from bugpilot.utils.proctitle import init_process_name

    init_process_name("BugPilot")
    ...
    from bugpilot.agentspec import DEFAULT_AGENT_FILE, OKABE_AGENT_FILE
    from bugpilot.app import BugPilotCLI, enable_logging
    from bugpilot.config import Config, load_config_from_string
    from bugpilot.exception import ConfigError
    from bugpilot.hooks import events as hook_events
    from bugpilot.metadata import load_metadata, save_metadata
    from bugpilot.session import Session
    ...
    # Redirection is installed later, right before BugPilotCLI.create(), so that
    ...
    async def _run(session_id: str | None, prefill_text: str | None = None) -> tuple[Session, int]:
        ...
        try:
            ...
            if local_add_dirs:
                from bugpilot.utils.path import is_within_directory
            ...
            # Redirect stderr *before* BugPilotCLI.create() so that MCP server
            ...
            instance = await BugPilotCLI.create(
                session,
                config=config,
    ...
    if _picker_mode:
        ...
        from bugpilot.utils.datetime import format_relative_time
    ...
    try:
        ...
        if debug:
            ...
            log_path = get_share_dir() / "logs" / "bugpilot.log"
            # In non-debug mode, print a concise error and point users to logs.
            _emit_fatal_error(
                f"{exc}\n"
                f"See logs: {log_path}\n"
                "Run with --debug for full traceback, or run bugpilot export to share diagnostics."
            )
        raise typer.Exit(code=1) from exc
    if switch_target in ("web", "vis"):
        from bugpilot.utils.logging import restore_stderr
        ...
        from bugpilot.utils.term import ensure_tty_sane
...
def term(
    ...
    """Run Toad TUI backed by BugPilot ACP server."""
...
def background_task_worker(
    ...
    from bugpilot.background import run_background_task_worker
    from bugpilot.utils.proctitle import set_process_title

    set_process_title("bugpilot-code-bg-worker")

    from bugpilot.app import enable_logging
...
def web_worker(session_id: str) -> None:
    ...
    set_process_title("bugpilot-code-worker")
...
if __name__ == "__main__":
    ...
    if "bugpilot.cli" not in sys.modules:
        sys.modules["bugpilot.cli"] = sys.modules[__name__]
