...
def _default_acp_command() -> list[str]:
    ...
    return [sys.executable, "-m", "bugpilot.cli", "acp"]


def _default_toad_command() -> list[str]:
    if sys.version_info < (3, 14):
        typer.echo("`bugpilot term` requires Python 3.14+ because Toad requires it.", err=True)
        raise typer.Exit(code=1)
    if importlib.util.find_spec("toad") is None:
        typer.echo(
            "Toad dependency is missing. Install bugpilot with Python 3.14+ to use `bugpilot term`.",
            err=True,
        )
        raise typer.Exit(code=1)
