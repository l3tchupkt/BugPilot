...
@cli.command(
    ...
    Examples:\n
      ...
      bugpilot mcp add --transport http context7 https://mcp.context7.com/mcp --header \"CONTEXT7_API_KEY: ctx7sk-your-key\"\n
      \n
...
def _has_oauth_tokens(server_url: str) -> bool:
    ...
    from bugpilot.mcp_oauth import has_mcp_oauth_tokens
    ...
    for name, server in servers.items():
        if "command" in server:
            ...
            if server.get("auth") == "oauth" and not _has_oauth_tokens(server["url"]):
                line += " [authorization required - run: bugpilot mcp auth " + name + "]"
        else:
            line = f"{name}: {server}"
        typer.echo(f"  {line}")
...
def mcp_auth(
    ...
    async def _auth() -> None:
        ...
        from bugpilot.mcp_oauth import prepare_mcp_server_config
...
def mcp_reset_auth(
    ...
    async def _reset_auth() -> None:
        from bugpilot.mcp_oauth import create_mcp_oauth_token_storage
