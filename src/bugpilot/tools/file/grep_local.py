...
from bugpilot.tools.utils import ToolResultBuilder, load_desc
from bugpilot.utils.aiohttp import new_client_session
from bugpilot.utils.logging import logger
from bugpilot.utils.path import normalize_user_path
from bugpilot.utils.sensitive import is_sensitive_file, sensitive_file_warning
...
RG_VERSION = "15.0.0"
RG_BASE_URL = "http://cdn.bugpilot.com/binaries/bugpilot/rg"
...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    assert bugpilot.__file__ is not None
    local_dep = Path(bugpilot.__file__).parent / "deps" / "bin" / bin_name
...
async def _download_and_install_rg(bin_name: str) -> Path:
    ...
    async with new_client_session(timeout=download_timeout) as session:
        with tempfile.TemporaryDirectory(prefix="bugpilot-rg-") as tmpdir:
