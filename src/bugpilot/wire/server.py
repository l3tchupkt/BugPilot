...
from bugpilot.soul.toolset import Toolset, WireExternalTool
from bugpilot.utils.aioqueue import Queue, QueueShutDown
...
class WireServer:
    ...
    def _approval_runtime(self) -> ApprovalRuntime | None:
        if isinstance(self._soul, Agent):
...
#         self._reader, self._writer = await acp.stdio_streams(limit=STDIO_BUFFER_LIMIT)
    ...
    async def _handle_initialize(
        ...
        toolset = None
        if isinstance(self._soul, Agent) and isinstance(self._soul.agent.toolset, Toolset):
        ...
        from bugpilot.constant import NAME, VERSION
        from bugpilot.hooks.config import HOOK_EVENT_TYPES
        from bugpilot.hooks.engine import WireHookHandle, WireHookSubscription
        from bugpilot.soul import wire_send
        from bugpilot.wire.protocol import WIRE_PROTOCOL_VERSION
        from bugpilot.wire.types import HookResolved, HookTriggered
    ...
    def _sync_ask_user_tool_visibility(self, toolset: Toolset) -> None:
        """Hide or unhide the AskUserQuestion tool based on client capabilities."""
        from bugpilot.tools.ask_user import NAME as ASK_USER_TOOL_NAME
    ...
    def _sync_plan_mode_tool_visibility(self, toolset: Toolset) -> None:
        """Hide or unhide plan mode tools based on client capabilities."""
        from bugpilot.tools.plan import NAME as EXIT_PLAN_MODE_TOOL_NAME
        from bugpilot.tools.plan.enter import NAME as ENTER_PLAN_MODE_TOOL_NAME
    ...
    async def _handle_prompt(
        ...
        runtime = self._soul.runtime if isinstance(self._soul, Agent) else None
        try:
            await run_soul(
                self._soul,
    ...
    async def _handle_steer(
        ...
        if not isinstance(self._soul, Agent) or not self._is_streaming:
            return JSONRPCErrorResponse(
                id=msg.id,
                error=JSONRPCErrorObject(
                    code=ErrorCodes.INVALID_STATE,
    ...
    async def _handle_replay(
        ...
        wire_file = self._soul.wire_file if isinstance(self._soul, Agent) else None
