...
from bugpilot.tools.display import DiffDisplayBlock
from bugpilot.ui.theme import get_diff_colors
from bugpilot.utils.rich.syntax import BugPilotSyntax
...
def _make_highlighter(path: str) -> BugPilotSyntax:
    """Create a BugPilotSyntax instance for highlighting code by file extension."""
    ext = path.rsplit(".", 1)[-1] if "." in path else ""
    return BugPilotSyntax("", ext if ext else "text")


def _highlight(highlighter: BugPilotSyntax, code: str) -> Text:
...
def _highlight_hunk(highlighter: BugPilotSyntax, hunk: list[DiffLine]) -> None:
