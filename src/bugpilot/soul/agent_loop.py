...
from bugpilot.background import build_active_task_snapshot
...
from bugpilot.skill import Skill, read_skill_text
from bugpilot.skill.flow import Flow, FlowEdge, FlowNode, parse_choice
from bugpilot.soul import (
    LLMNotSet,
    LLMNotSupported,
    MaxStepsReached,
    Soul,
...
from bugpilot.soul.compaction import (
...
from bugpilot.soul.dynamic_injection import (
    DynamicInjection,
...
from bugpilot.soul.message import check_message, system, system_reminder, tool_result_to_message
from bugpilot.soul.slash import registry as soul_slash_registry
from bugpilot.soul.toolset import Toolset
from bugpilot.tools.dmail import NAME as SendDMail_NAME
from bugpilot.tools.utils import ToolRejectedError
from bugpilot.utils.logging import logger
from bugpilot.utils.slashcmd import SlashCommand, parse_slash_command_call
...
def is_retryable_api_error(e: Exception) -> bool:
    ...
    Deliberately separate from ``Agent._is_retryable_error``, which drives
    ...
    def __init__(
        ...
        *,
        ...
        if self._plan_session_id is not None and self._runtime.session.state.plan_slug is not None:
            from bugpilot.tools.plan.heroes import seed_slug_cache
...
#         set_current_trace_id(trace_id)
    ...
    def set_hook_engine(self, engine: HookEngine) -> None:
        ...
        if isinstance(self._agent.toolset, Toolset):
    ...
    def _bind_plan_mode_tools(self) -> None:
        """Bind plan mode state to tools that support it."""
        if not isinstance(self._agent.toolset, Toolset):
            return
        ...
        from bugpilot.tools.ask_user import AskUserQuestion
    ...
    def _ensure_plan_session_id(self) -> None:
        ...
        if self._plan_session_id is None:
            ...
            from bugpilot.tools.plan.heroes import get_or_create_slug
    ...
    def get_plan_file_path(self) -> Path | None:
        ...
        if self._plan_session_id is None:
            return None
        from bugpilot.tools.plan.heroes import get_plan_file_path
    ...
    def read_current_plan(self) -> str | None:
        ...
        if self._plan_session_id is None:
            return None
        from bugpilot.tools.plan.heroes import read_plan_file
    ...
    async def run(
        ...
        *,
        ...
        try:
            ...
            from bugpilot.soul.toolset import set_session_id

            set_session_id(self._runtime.session.id)

            from bugpilot.hooks import events
            ...
            if not command_call:
                ...
                if session.state.custom_title is None:
                    ...
                    if title:
                        from bugpilot.session_state import (
                            load_session_state,
                            save_session_state,
                        )
...
#             track("skill_invoked", skill_name=_skill.name)
    ...
    async def _agent_loop(self) -> TurnOutcome:
        ...
        while True:
            ...
            try:
                ...
                # --- StopFailure hook ---
                from bugpilot.hooks import events as _hook_events
            ...
            if back_to_the_future is not None:
                ...
                # Toolset.begin_step resets dedup/repeat state when the
    ...
    async def _step(self) -> StepOutcome | None:
        ...
        assert self._runtime.llm is not None
        provider = self._runtime.llm.provider
        ...
        async def _run_step_once() -> StepResult:
            ...
            async for chunk in stream:
                if chunk.usage:
                    usage = chunk.usage
                if chunk.text_delta:
                    part = TextPart(text=chunk.text_delta)
                    message.content.append(part)
                    wire_send(part)
                if chunk.reasoning_delta:
                    part = ThinkPart(think=chunk.reasoning_delta)
                    message.content.append(part)
                    wire_send(part)
                if chunk.tool_call:
                    tc = chunk.tool_call
...
#             track("turn_force_stopped", reason="tool_call_repeat", step_no=self._current_step_no)
    ...
    def _compute_completion_overrides(
        ...
        *,
        ...
        Returns request-scoped BugPilot generation overrides, or ``None`` if no overrides
...
class FlowRunner:
    ...
    async def run(self, soul: Agent, args: str) -> None:
        if args.strip():
