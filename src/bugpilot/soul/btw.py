...
from bugpilot.wire.types import BtwBegin, BtwEnd, TextPart
...
def _build_btw_context(soul: Agent, question: str) -> tuple[str, list[Message], _DenyAllToolset]:
    ...
    as ``Agent._step`` so the LLM provider can reuse the prompt cache.
    """
...
async def execute_side_question(
    ...
    Args:
        soul: The Agent instance (for context and chat_provider access).
...
async def run_side_question(soul: Agent, question: str) -> None:
