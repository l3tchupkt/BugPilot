...
import bugpilot.prompts as prompts
from bugpilot import logger
from bugpilot.soul import wire_send
from bugpilot.soul.agent import load_agents_md
from bugpilot.soul.context import Context
from bugpilot.soul.dynamic_injections.afk_mode import AFK_DISABLED_REMINDER
from bugpilot.soul.message import system, system_reminder
from bugpilot.utils.export import is_sensitive_file
from bugpilot.utils.path import sanitize_cli_path, shorten_home
from bugpilot.utils.slashcmd import SlashCommandRegistry
from bugpilot.wire.types import StatusUpdate, TextPart

if TYPE_CHECKING:
    from bugpilot.soul.agent_loop import Agent

type SoulSlashCmdFunc = Callable[[Agent, str], None | Awaitable[None]]
"""
A function that runs as a Agent-level slash command.
...
@registry.command
async def init(soul: Agent, args: str):
    """Analyze the codebase and generate an `AGENTS.md` file"""
    from bugpilot.soul.agent_loop import Agent

    with tempfile.TemporaryDirectory() as temp_dir:
        tmp_context = Context(file_backend=Path(temp_dir) / "context.jsonl")
        tmp_soul = Agent(soul.agent, context=tmp_context)
        await tmp_soul.run(prompts.INIT)

    agents_md = await load_agents_md(soul.runtime.builtin_args.BUGPILOT_WORK_DIR)
...
@registry.command
async def compact(soul: Agent, args: str):
...
async def clear(soul: Agent, args: str):
    """Clear the context"""
...
@registry.command
async def yolo(soul: Agent, args: str):
...
@registry.command
async def afk(soul: Agent, args: str):
...
@registry.command
async def plan(soul: Agent, args: str):
...
async def add_dir(soul: Agent, args: str):
    """Add a directory to the workspace. Usage: /add-dir <path>. Run without args to list added dirs"""  # noqa: E501
    from kaos.path import KaosPath

    from bugpilot.utils.path import is_within_directory, list_directory
    ...
    work_dir = soul.runtime.builtin_args.BUGPILOT_WORK_DIR
...
@registry.command
async def export(soul: Agent, args: str):
...
async def import_context(soul: Agent, args: str):
