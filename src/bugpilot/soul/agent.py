...
from bugpilot.agentspec import load_agent_spec
from bugpilot.approval_runtime import ApprovalRuntime
from bugpilot.background import BackgroundTaskManager
from bugpilot.config import Config
from bugpilot.exception import MCPConfigError, SystemPromptTemplateError
...
from bugpilot.soul.approval import Approval, ApprovalState
from bugpilot.soul.denwarenji import DenwaRenji
from bugpilot.soul.toolset import Toolset
from bugpilot.subagents.models import AgentTypeDefinition, ToolPolicy
...
from bugpilot.utils.path import find_project_root, is_within_directory, list_directory
...
async def load_agents_md(work_dir: KaosPath) -> str | None:
    ...
    1. ``.bugpilot/AGENTS.md``  — project-local bugpilot config (highest priority)
    2. ``AGENTS.md``        — standard location
    3. ``agents.md``        — lowercase variant (mutually exclusive with 2)

    Within a single directory, ``.bugpilot/AGENTS.md`` and ``AGENTS.md``/``agents.md``
    are **both** loaded (with ``.bugpilot/`` first), but ``AGENTS.md`` and ``agents.md``
...
#     oauth: OAuthManager
    ...
    """HookEngine instance, set by BugPilotCLI after soul creation."""
...
#         oauth: OAuthManager,
        ...
        return Runtime(
            ...
            session=session,
            builtin_args=BuiltinSystemPromptArgs(
                BUGPILOT_NOW=datetime.now().astimezone().isoformat(),
                BUGPILOT_WORK_DIR=session.work_dir,
                BUGPILOT_WORK_DIR_LS=ls_output,
                BUGPILOT_AGENTS_MD=agents_md or "",
                BUGPILOT_SKILLS=skills_formatted or "No skills found.",
                BUGPILOT_ADDITIONAL_DIRS_INFO=additional_dirs_info,
                BUGPILOT_OS=environment.os_kind,
                BUGPILOT_SHELL=f"{environment.shell_name} (`{environment.shell_path}`)",
            ),
            denwa_renji=DenwaRenji(),
...
async def load_agent(
    ...
    *,
    ...
    Raises:
        ...
        SystemPromptTemplateError(BugPilotError, ValueError): When the system prompt template
            is invalid.
    ...
    # Load plugin tools
    from bugpilot.plugin.manager import get_plugins_dir
    from bugpilot.plugin.tool import load_plugin_tools
