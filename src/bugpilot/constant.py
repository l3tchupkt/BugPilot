...
def get_version() -> str:
    ...
    return metadata.version("bugpilot")


@cache
def get_user_agent() -> str:
    return f"BugPilotCLI/{get_version()}"
...
def get_build_sha() -> str:
    ...
    Priority:
    1. BUGPILOT_BUILD_SHA environment variable (dev / CI override)
    ...
    """
    if build_id := os.environ.get("BUGPILOT_BUILD_SHA", "").strip():
        return build_id

    try:
        from bugpilot._build_info import BUILD_SHA  # type: ignore[reportMissingImports]
