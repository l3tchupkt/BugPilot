...
from bugpilot.skill.flow import Flow, FlowError
from bugpilot.skill.flow.d2 import parse_d2_flowchart
from bugpilot.skill.flow.mermaid import parse_mermaid_flowchart
from bugpilot.utils.frontmatter import parse_frontmatter
...
- ``builtin``: bundled with bugpilot
- ``user``: from the user's home (``~/.bugpilot/skills``, ``~/.agents/skills``, ...)
- ``project``: from the current project's working directory
  (``<work_dir>/.bugpilot/skills``, ``<work_dir>/.agents/skills``, ...)
...
def get_builtin_skills_dir() -> Path:
    ...
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        ...
        return Path(meipass) / "bugpilot" / "skills"
...
def _get_user_brand_skills_dir_candidates() -> tuple[KaosPath, ...]:
    ...
    Brand group: ``~/.bugpilot/skills`` > ``~/.claude/skills`` > ``~/.codex/skills``
    """
    return (
        KaosPath.home() / ".bugpilot" / "skills",
...
async def find_user_skills_dirs(
    *,
    ...
    directories are included (priority order: bugpilot > claude > codex).
    """
    dirs: list[KaosPath] = []
    if merge_brands:
...
async def find_project_skills_dirs(
    ...
    *,
    ...
    of ``work_dir``), so launching bugpilot from a subdirectory — for example
    ...
    directories are included (priority order: bugpilot > claude > codex).
    """
    from bugpilot.utils.path import find_project_root
...
def _first_meaningful_line(content: str) -> str | None:
    ...
    :mod:`bugpilot.utils.frontmatter`). A malformed frontmatter opener that
    ...
    """
    from bugpilot.utils.frontmatter import strip_frontmatter
