...
from bugpilot.approval_runtime import get_current_approval_source_or_none
from bugpilot.soul import MaxStepsReached, RunCancelled
...
from bugpilot.wire.types import ApprovalRequest, TextPart
...
async def test_foreground_agent_explicit_timeout_returns_tool_error(
    ...
    monkeypatch.setattr("bugpilot.subagents.builder.load_agent", fake_load_agent)
    monkeypatch.setattr("bugpilot.subagents.runner.run_soul", fake_run_soul_hang)
...
async def test_foreground_agent_internal_timeout_with_explicit_deadline(
    ...
    monkeypatch.setattr("bugpilot.subagents.builder.load_agent", fake_load_agent)
    monkeypatch.setattr("bugpilot.subagents.runner.run_soul", fake_run_soul_internal_timeout)
...
async def test_agent_tool_returns_error_when_final_response_is_none(
    ...
    monkeypatch.setattr(
        "bugpilot.subagents.runner.run_with_summary_continuation",
...
async def test_background_agent_marks_failed_when_final_response_is_none(
    ...
    monkeypatch.setattr(
        "bugpilot.background.agent_runner.run_with_summary_continuation",
        fake_run_with_summary,
    )
...
async def test_foreground_runner_hook_trigger_exception_marks_instance_failed(
    ...
    monkeypatch.setattr(
        "bugpilot.hooks.events.subagent_start",
