"""E2E regression test for BugPilot-compatible endpoints that reject empty assistant text.
...
3. Some BugPilot-compatible gateways reject the second request if the prior assistant
...
async def mock_server() -> AsyncIterator[MockServer]:
    server_holder: _MockServer | None = None
    ...
    server_holder = _MockServer(f"http://127.0.0.1:{port}")
...
def _write_bugpilot_config(config_path: Path, *, base_url: str) -> None:
    config_path.write_text(
        json.dumps(
            {
                "default_model": "mock-bugpilot",
                "models": {
                    "mock-bugpilot": {
                        "provider": "mock-bugpilot-provider",
                        "model": "bugpilot-for-coding",
                        "max_context_size": 100000,
                    }
                },
                "providers": {
                    "mock-bugpilot-provider": {
                        "type": "bugpilot",
                        "base_url": base_url,
...
async def _run_bugpilot_print_json(
    *,
    ...
    env["BUGPILOT_DISABLE_TELEMETRY"] = "1"
    env["COLUMNS"] = "120"
    env["LINES"] = "40"
...
async def test_bugpilot_compat_endpoint_accepts_tool_call_history_without_empty_content(
    ...
    for body in mock_server.requests:
        _assert_thinking_request_body(body, expected_type="disabled")
    assistant_message = _find_assistant_tool_call_message(mock_server.requests[1])
