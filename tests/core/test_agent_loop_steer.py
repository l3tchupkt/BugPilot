...
from bugpilot.wire.types import ImageURLPart, SteerInput, StepBegin, TextPart, TurnBegin, TurnEnd
...
async def test_consume_pending_steers_appends_history_before_emitting_wire_event(
    ...
    monkeypatch.setattr(agent_loop_module, "wire_send", fake_wire_send)
...
async def test_consume_pending_steers_does_not_emit_wire_event_for_unsupported_media(
    ...
    soul = _make_soul(_runtime_with_llm(runtime, _llm_with_capabilities(runtime, set())), tmp_path)
    sent: list[SteerInput] = []
    monkeypatch.setattr(agent_loop_module, "wire_send", lambda msg: sent.append(msg))
...
async def test_agent_loop_injects_steer_between_completed_steps(
    ...
    async def fake_step():
        ...
        return agent_loop_module.StepOutcome(
...
async def test_step_merges_plain_steer_with_dynamic_injection_in_model_history(
    ...
    monkeypatch.setattr(agent_loop_module.kosong, "step", fake_kosong_step)
    monkeypatch.setattr(agent_loop_module, "wire_send", lambda _msg: None)
