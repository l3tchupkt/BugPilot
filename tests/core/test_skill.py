...
async def test_resolve_skills_roots_uses_layers(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("BUGPILOT_SHARE_DIR", str(tmp_path / "share"))
...
async def test_find_user_skills_dirs_empty_generic_does_not_shadow_brand(monkeypatch, tmp_path):
    """Core bug: empty ~/.config/agents/skills should NOT hide ~/.bugpilot/skills."""
    home_dir = tmp_path / "home"
    generic_dir = home_dir / ".config" / "agents" / "skills"
    generic_dir.mkdir(parents=True)  # exists but empty

    brand_dir = home_dir / ".bugpilot" / "skills"
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_brand_group_prefers_bugpilot_over_claude(
    monkeypatch, tmp_path
):
    """Brand group: ~/.bugpilot/skills takes priority over ~/.claude/skills."""
    home_dir = tmp_path / "home"
    bugpilot_dir = home_dir / ".bugpilot" / "skills"
    bugpilot_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    claude_dir.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home_dir)

    dirs = await find_user_skills_dirs()
    # Only bugpilot should be selected (first existing in brand group)
    assert KaosPath.unsafe_from_local_path(bugpilot_dir) in dirs
...
async def test_find_project_skills_dirs_merge(tmp_path):
    ...
    generic_dir.mkdir(parents=True)
    brand_dir = work_dir / ".bugpilot" / "skills"
    brand_dir.mkdir(parents=True)
...
@pytest.mark.asyncio
async def test_find_project_skills_dirs_brand_prefers_bugpilot(tmp_path):
    """Project layer brand group: .bugpilot/skills wins over .claude/skills."""
    work_dir = tmp_path / "project"
    bugpilot_dir = work_dir / ".bugpilot" / "skills"
    bugpilot_dir.mkdir(parents=True)
    ...
    dirs = await find_project_skills_dirs(KaosPath.unsafe_from_local_path(work_dir))
    assert len(dirs) == 1
    assert dirs[0] == KaosPath.unsafe_from_local_path(bugpilot_dir)
...
async def test_resolve_skills_roots_merges_user_and_project(monkeypatch, tmp_path):
    ...
    user_brand = home_dir / ".bugpilot" / "skills"
    ...
    proj_brand = work_dir / ".bugpilot" / "skills"
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_bugpilot_and_claude(monkeypatch, tmp_path):
    """merge_brands=True: bugpilot + claude both exist → both returned, bugpilot first."""
    home_dir = tmp_path / "home"
    bugpilot_dir = home_dir / ".bugpilot" / "skills"
    bugpilot_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    ...
    # bugpilot before claude
    bugpilot_idx = dirs.index(KaosPath.unsafe_from_local_path(bugpilot_dir))
    claude_idx = dirs.index(KaosPath.unsafe_from_local_path(claude_dir))
    assert bugpilot_idx < claude_idx


@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_all_three(monkeypatch, tmp_path):
    """merge_brands=True: all three brand dirs → [bugpilot, claude, codex]."""
    home_dir = tmp_path / "home"
    bugpilot_dir = home_dir / ".bugpilot" / "skills"
    bugpilot_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    ...
    assert brand_dirs[0] == KaosPath.unsafe_from_local_path(bugpilot_dir)
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_same_skill_bugpilot_wins(monkeypatch, tmp_path):
    """merge_brands=True + same skill name → bugpilot version wins via discover."""
    home_dir = tmp_path / "home"
    bugpilot_dir = home_dir / ".bugpilot" / "skills"
    bugpilot_dir.mkdir(parents=True)
    _write_skill(
        bugpilot_dir / "deploy",
        "---\nname: deploy\ndescription: bugpilot deploy\n---\n",
    )
    claude_dir = home_dir / ".claude" / "skills"
    claude_dir.mkdir(parents=True)
    _write_skill(
    ...
    skills = await discover_skills_from_roots(_as_user(dirs))
    assert len(skills) == 1
    assert skills[0].description == "bugpilot deploy"
...
def test_get_builtin_skills_dir_frozen_env(monkeypatch, tmp_path):
    ...
    assert result == fake_meipass / "bugpilot" / "skills"
...
def test_get_builtin_skills_dir_normal_env():
    ...
    assert result.parent.name == "bugpilot"
...
async def test_resolve_skills_roots_passes_merge_brands(monkeypatch, tmp_path):
    ...
    # Without merge_brands: only bugpilot
    roots_default = _roots_of(await resolve_skills_roots(KaosPath.unsafe_from_local_path(work_dir)))
    assert KaosPath.unsafe_from_local_path(bugpilot_dir) in roots_default
    ...
    assert KaosPath.unsafe_from_local_path(bugpilot_dir) in roots_merged
...
async def test_project_scope_skill_overrides_builtin_with_same_name(monkeypatch, tmp_path):
    ...
    import bugpilot.skill as skill_mod
    ...
    work_dir = tmp_path / "project"
    proj_dir = work_dir / ".bugpilot" / "skills"
...
async def test_user_scope_skill_overrides_builtin_with_same_name(monkeypatch, tmp_path):
    ...
    home_dir = tmp_path / "home"
    user_dir = home_dir / ".bugpilot" / "skills"
    user_dir.mkdir(parents=True)
    (user_dir / "foo").mkdir()
...
async def test_extra_skill_dirs_dedup_with_auto_discovery_overlap(monkeypatch, tmp_path):
    ...
    Scenario: user puts ``~/.bugpilot/skills`` (already auto-discovered as the
...
# the work directory. Launching bugpilot from a subdirectory of a monorepo
...
async def test_find_project_skills_dirs_walks_up_to_git_root(tmp_path):
    ...
    defined at the repo root — otherwise running bugpilot inside a monorepo
...
# ~/.bugpilot/skills and ~/.claude/skills after migrating from Claude Code)
# previously lost visibility of everything in .claude/skills because the
# default "first brand wins" behaviour would stop at .bugpilot/skills. Flipping
...
async def test_default_config_effectively_merges_user_brand_skill_dirs(monkeypatch, tmp_path):
    ...
    both ``~/.bugpilot/skills`` and ``~/.claude/skills`` are surfaced. If the
    ...
    assert KaosPath.unsafe_from_local_path(bugpilot_dir) in roots
