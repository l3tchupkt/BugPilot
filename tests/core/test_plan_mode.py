...
from bugpilot.tools.plan import ExitPlanMode, Params, PlanOption
from bugpilot.tools.plan.enter import _DESCRIPTION, EnterPlanMode
from bugpilot.tools.plan.heroes import (
    _slug_cache,
    get_or_create_slug,
    get_plan_file_path,
    read_plan_file,
...
class TestGetOrCreateSlug:
    ...
    def test_collision_fallback(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("bugpilot.tools.plan.heroes.HERO_NAMES", ["a", "b"])
...
def _mock_wire_and_tool_call(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("bugpilot.tools.plan.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("bugpilot.tools.plan.wire_send", lambda msg: None)
    monkeypatch.setattr("bugpilot.tools.plan.enter.get_wire_or_none", lambda: wire_mock)
    monkeypatch.setattr("bugpilot.tools.plan.enter.wire_send", lambda msg: None)
    tc = ToolCall(id="test-tc", function=ToolCall.FunctionBody(name="ExitPlanMode", arguments=None))
    monkeypatch.setattr("bugpilot.tools.plan.get_current_tool_call_or_none", lambda: tc)
    monkeypatch.setattr("bugpilot.tools.plan.enter.get_current_tool_call_or_none", lambda: tc)
    return wire_mock
...
class TestExitPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_exit_tool(tmp_path)
        monkeypatch.setattr("bugpilot.tools.plan.get_wire_or_none", lambda: None)
...
class TestEnterPlanModeHappyPaths:
    ...
    async def test_wire_unavailable(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        tool, _, _ = _setup_enter_tool(tmp_path)
        monkeypatch.setattr("bugpilot.tools.plan.enter.get_wire_or_none", lambda: None)
...
# ---------------------------------------------------------------------------
# Agent — plan mode state management
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Agent — plan_session_id cross-process persistence
# ---------------------------------------------------------------------------


class TestAgentPlanSessionPersistence:
    """Tests that plan_session_id survives simulated process restarts."""

    async def test_plan_session_id_survives_restart(
        self, runtime: Runtime, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Second Agent created from same session state gets same plan file path."""
        monkeypatch.setattr("bugpilot.tools.plan.heroes.PLANS_DIR", tmp_path)
        ...
        # New Agent reads from same (already-saved) session state and re-seeds cache
