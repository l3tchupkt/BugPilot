...
from bugpilot.agentspec import DEFAULT_AGENT_FILE, load_agent_spec
from bugpilot.exception import AgentSpecError
...
def test_load_default_agent_spec():
    ...
    assert spec.tools == snapshot(
        [
            ...
            "bugpilot.tools.ask_user:AskUserQuestion",
            "bugpilot.tools.todo:SetTodoList",
            "bugpilot.tools.shell:Shell",
            "bugpilot.tools.background:TaskList",
            "bugpilot.tools.background:TaskOutput",
            "bugpilot.tools.background:TaskStop",
...
def test_load_agent_spec_with_exclude_tools(agent_file_with_tools: Path):
    ...
    assert spec.tools == snapshot(["bugpilot.tools.think:Think", "bugpilot.tools.shell:Shell"])
    assert spec.exclude_tools == snapshot(["bugpilot.tools.shell:Shell"])
...
""")
        ...
        assert spec.exclude_tools == snapshot(
            ["bugpilot.tools.web:SearchWeb", "bugpilot.tools.web:FetchURL"]
        )
...
agent:
  name: "Test Agent"
  system_prompt_path: ./system.md
  tools: ["bugpilot.tools.think:Think", "bugpilot.tools.shell:Shell"]
  exclude_tools: ["bugpilot.tools.shell:Shell"]
""")
