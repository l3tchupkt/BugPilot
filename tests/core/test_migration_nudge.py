...
from bugpilot.ui.shell import migration_nudge as mn
...
def test_install_run_command_wraps_powershell_on_windows():
    ...
    assert "irm https://code.bugpilot.com/bugpilot-code/install.ps1 | iex" in win


def test_verify_command_per_platform():
    assert mn.verify_command("darwin") == "which bugpilot"
    assert mn.verify_command("linux") == "which bugpilot"
    assert mn.verify_command("win32") == "where bugpilot"
