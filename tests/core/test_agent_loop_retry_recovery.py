...
from bugpilot.config import LLMModel, LLMProvider, OAuthRef
...
def _make_soul(runtime: Runtime, llm: LLM, tmp_path: Path) -> tuple[Agent, Context]:
    agent = Agent(
...
async def test_step_connection_recovery_then_401_triggers_oauth_refresh(
    ...
    oauth_model = LLMModel(
        provider="managed:bugpilot-code",
        model="bugpilot-for-coding",
        max_context_size=100_000,
    )
    runtime.config.providers[oauth_model.provider] = oauth_provider
    runtime.config.models["bugpilot-code/bugpilot-for-coding"] = oauth_model
