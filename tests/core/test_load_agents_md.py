...
from bugpilot.soul.agent import _AGENTS_MD_MAX_BYTES, load_agents_md
...
async def test_kimi_dir_and_root_both_loaded(temp_work_dir: KaosPath):
    ...
    assert content is not None
    assert content.index("bugpilot agents") < content.index("root agents")
...
async def test_kimi_dir_in_parent(temp_work_dir: KaosPath):
    ...
    assert content is not None
    assert "parent bugpilot" in content
    assert "child root" in content
    assert content.index("parent bugpilot") < content.index("child root")
