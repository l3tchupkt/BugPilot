#!/usr/bin/env python3
"""Clean up .bugpilot sessions whose workdir is under a temporary directory.

This script handles two cases:
  1. Entries in bugpilot.json whose path is a tmp directory -> remove entry + session dir.
  2. Orphan session directories on disk that have no matching bugpilot.json entry
...
BUGPILOT_DIR = Path.home() / ".bugpilot"
METADATA_FILE = BUGPILOT_DIR / "bugpilot.json"
SESSIONS_DIR = BUGPILOT_DIR / "sessions"
...
def main() -> None:
    ...
    # --- Phase 1: tmp entries in bugpilot.json ---
    ...
    # --- Phase 2: orphan directories (on disk but not in bugpilot.json) ---
