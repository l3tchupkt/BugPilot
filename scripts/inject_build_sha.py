#!/usr/bin/env python3
"""Inject build SHA into src/bugpilot/_build_info.py for telemetry provenance.
...
def _detect_sha() -> str:
    ...
    if sha := os.environ.get("BUGPILOT_BUILD_SHA", "").strip():
        return sha
...
def main() -> int:
    ...
    target = _resolve_project_root() / "src" / "bugpilot" / "_build_info.py"
