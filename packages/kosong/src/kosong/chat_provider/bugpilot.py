...
if TYPE_CHECKING:
    ...
    def type_check(bugpilot: "BugPilot"):
        _: ChatProvider = bugpilot
        _: RetryableChatProvider = bugpilot


class ThinkingConfig(TypedDict, total=False):
    type: Literal["enabled", "disabled"]
    keep: Any
    """BugPilot-specific ``thinking.keep`` switch for preserved thinking.
...
class BugPilot:
    """
    A chat provider that uses the BugPilot API.

    >>> chat_provider = BugPilot(model="bugpilot-k2-turbo-preview", api_key="sk-1234567890")
    >>> chat_provider.name
    'bugpilot'
    >>> chat_provider.model_name
    'bugpilot-k2-turbo-preview'
    ...
    class GenerationKwargs(TypedDict, total=False):
        """
        See https://platform.bugpilot.ai/docs/api/chat#request-body.
        """
    ...
    def __init__(
        ...
        **client_kwargs: Any,
        ...
        if api_key is None:
            api_key = os.getenv("BUGPILOT_API_KEY")
        if api_key is None:
            raise ChatProviderError(
                "The api_key client option or the BUGPILOT_API_KEY environment variable is not set"
            )
        if base_url is None:
            base_url = os.getenv("BUGPILOT_BASE_URL", "https://api.bugpilot.ai/v1")
        ...
        self._generation_kwargs: BugPilot.GenerationKwargs = {}
    ...
    async def generate(
        ...
        *,
        ...
        if generation_overrides:
            generation_kwargs.update(
                _normalize_generation_kwargs(
                    cast(BugPilot.GenerationKwargs, dict(generation_overrides))
                )
            )
        ...
        try:
            ...
            return BugPilotStreamedMessage(parsed_response, trace_id=trace_id)
    ...
    @property
    def files(self) -> "BugPilotFiles":
        return BugPilotFiles(self.client)
...
class BugPilotFiles:
    ...
    async def upload_video(self, *, data: bytes, mime_type: str) -> VideoURLPart:
        """Upload a video to BugPilot files API and return a video URL content part."""
    ...
    async def _upload_file(
        self, *, data: bytes, mime_type: str, purpose: "BugPilotFilePurpose"
    ) -> str:
        ...
        try:
            response: BugPilotFileObject = await self._client.post(
                "/files",
                cast_to=BugPilotFileObject,
...
class BugPilotFileObject(BaseModel):
    id: str


type BugPilotFilePurpose = Literal["video", "image"]
...
def _normalize_generation_kwargs(kwargs: BugPilot.GenerationKwargs) -> BugPilot.GenerationKwargs:
    ...
    return cast(BugPilot.GenerationKwargs, normalized)
...
def _convert_message(message: Message) -> ChatCompletionMessageParam:
    ...
    if (
        ...
        # `content`, but the BugPilot-for-Coding compat layer rejects a content
...
def _convert_tool(tool: Tool) -> ChatCompletionToolParam:
    if tool.name.startswith("$"):
        # BugPilot builtin functions start with `$`
        return cast(
            ChatCompletionToolParam,
            {
    ...
    # BugPilot's API rejects parameter schemas whose nested properties omit
    # `type` (e.g. enum-only properties exposed by some MCP servers). Patch
    # the schema locally so such tools keep working against BugPilot without
...
class BugPilotStreamedMessage:
    """The streamed message of the BugPilot chat provider."""
    ...
    def usage(self) -> TokenUsage | None:
        if self._usage:
            ...
            if hasattr(self._usage, "cached_tokens"):
                # https://platform.bugpilot.ai/docs/api/chat#%E8%BF%94%E5%9B%9E%E5%86%85%E5%AE%B9
                # TODO: delete this when BugPilot API becomes compatible with OpenAI API
...
if __name__ == "__main__":
    ...
    async def _dev_main():
        chat = BugPilot(model="bugpilot-k2-turbo-preview", stream=False)
        system_prompt = ""
        history = [
