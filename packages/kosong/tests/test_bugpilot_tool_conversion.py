"""Tests for `_convert_tool` in the BugPilot chat provider.

These guard the BugPilot-specific schema quirk: BugPilot rejects tool
parameter schemas whose nested properties omit `type`, even though such
schemas are valid JSON Schema and accepted by OpenAI/Anthropic. The BugPilot
...
from kosong.chat_provider.bugpilot import _convert_tool  # pyright: ignore[reportPrivateUsage]
from kosong.tooling import Tool


def _jetbrains_like_tool() -> Tool:
    """A Tool shaped like the real jetbrains-rider MCP tool that triggered
    the BugPilot 400 — `truncateMode` has `enum` but no `type`."""
    return Tool(
...
def test_convert_tool_preserves_builtin_function_shape() -> None:
    """BugPilot builtin functions (names starting with `$`) don't carry
