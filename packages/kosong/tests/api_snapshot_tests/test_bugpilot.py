"""Snapshot tests for BugPilot chat provider."""
...
from kosong.chat_provider.bugpilot import BugPilot, BugPilotStreamedMessage
...
async def test_bugpilot_message_conversion():
    with respx.mock(base_url="https://api.bugpilot.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response("bugpilot-k2"))
        )
        provider = BugPilot(model="bugpilot-k2-turbo-preview", api_key="test-key", stream=False)
...
async def test_bugpilot_generation_kwargs():
    with respx.mock(base_url="https://api.bugpilot.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response())
        )
        provider = BugPilot(
            model="bugpilot-k2-turbo-preview", api_key="test-key", stream=False
...
async def test_bugpilot_default_omits_completion_cap():
    with respx.mock(base_url="https://api.bugpilot.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response())
        )
...
async def test_bugpilot_max_tokens_alias_preserves_explicit_none():
    with respx.mock(base_url="https://api.bugpilot.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response())
        )
        provider = (
            BugPilot(model="bugpilot-k2-turbo-preview", api_key="test-key", stream=False)
...
def test_bugpilot_max_tokens_alias_defers_to_canonical_key():
    provider = BugPilot(
        model="bugpilot-k2-turbo-preview", api_key="test-key", stream=False
...
async def test_bugpilot_generation_overrides_per_call():
...
async def test_bugpilot_generation_overrides_normalize_max_tokens_alias():
...
async def test_bugpilot_generation_overrides_take_precedence_over_provider_kwargs():
...
async def test_bugpilot_with_thinking_omits_legacy_reasoning_effort(
...
def test_bugpilot_thinking_effort_preserves_caller_value_without_mapping():
    provider = BugPilot(model="bugpilot-k2-turbo-preview", api_key="test-key", stream=False)
...
def test_bugpilot_explicit_legacy_reasoning_effort_is_independent_from_thinking_state():
    provider = BugPilot(
        model="bugpilot-k2-turbo-preview", api_key="test-key", stream=False
...
async def test_bugpilot_with_extra_body_thinking_deep_merge():
...
async def test_bugpilot_with_extra_body_thinking_empty_dict_is_noop():
...
async def test_bugpilot_with_extra_body_thinking_starts_from_empty_dict():
...
async def test_bugpilot_with_extra_body_non_thinking_key_shallow_merge():
    ...
    return ChatCompletionChunk.model_validate(
        {
            ...
            "created": 1,
            "model": "bugpilot-k2-thinking",
            "choices": [{"index": 0, "delta": delta, "finish_reason": finish_reason}],
        }
    )


async def test_bugpilot_stream_preserves_empty_reasoning_delta():
    ...
    stream = BugPilotStreamedMessage(_aiter())  # type: ignore[arg-type]
    parts = [part async for part in stream]
    assert parts[0] == ThinkPart(think="")
    assert isinstance(parts[1], ToolCall)


async def test_bugpilot_non_stream_preserves_empty_reasoning_content():
    ...
    stream = BugPilotStreamedMessage(response)
    parts = [part async for part in stream]
    assert parts[0] == ThinkPart(think="")
    assert parts[1] == TextPart(text="Done.")


async def test_bugpilot_absent_reasoning_field_still_yields_no_thinkpart():
