...
header. It must be captured by the BugPilot provider (both success and error
...
URL = "https://api.bugpilot.ai/v1/chat/completions"
...
@pytest.mark.asyncio
async def test_bugpilot_captures_trace_id_header():
    with respx.mock:
        respx.post(URL).mock(
        ...
        provider = BugPilot(model="test-model", api_key="token", stream=False)
...
@pytest.mark.asyncio
async def test_bugpilot_trace_id_none_without_header():
    with respx.mock:
...
@pytest.mark.asyncio
async def test_bugpilot_streaming_captures_trace_id():
    ...
    with respx.mock:
        ...
        provider = BugPilot(model="test-model", api_key="token", stream=True)
        stream = await provider.generate("", [], [])
        assert stream.trace_id == TRACE_ID
...
@pytest.mark.asyncio
async def test_bugpilot_streaming_uses_stream_response_headers():
    class DelayedStream:
...
@pytest.mark.asyncio
async def test_bugpilot_accepts_async_raw_response_parse():
